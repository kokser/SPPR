...
Процедура СохранитьИзмененияПатча(Параметры) Экспорт
	...
	// Получение дополнительных параметров для базовых версий патча.
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", Параметры.Версия);
	Запрос.Текст =
		"ВЫБРАТЬ
	...
	ПараметрыДляБазовых = Новый Структура;
	ПараметрыДляБазовых.Вставить("ЭтоБиблиотека", ЭтоБиблиотека);
	ПараметрыДляБазовых.Вставить("Версия", Параметры.Версия);
	ПараметрыДляБазовых.Вставить("Патч", Параметры.Патч);
	ПараметрыДляБазовых.Вставить("ДанныеПатча", ДанныеПатча);
	...
	Попытка
		...
		// Расчет применимости.
		СвойстваВерсииПатча = ПатчиСлужебный.ЗначенияСвойствВерсииПатча(ВерсияПатча.УникальныйИдентификатор, "РазделПроекта");
		ПараметрыРасчетаПрименимости = ПатчиСлужебный.ПараметрыРасчетаПрименимости();
		ПараметрыРасчетаПрименимости.ЭтоБиблиотека = ЭтоБиблиотека;
		ПараметрыРасчетаПрименимости.РазделПроекта = СвойстваВерсииПатча.РазделПроекта;
		ЗаполнитьЗначенияСвойств(ПараметрыРасчетаПрименимости, ВерсияПатча);
		ИнформацияОПрименимости = ПатчиСлужебный.ПрименимостьПатча(ПараметрыРасчетаПрименимости, Ложь);
		ПараметрыДляБазовых.Вставить("ИнформацияОПрименимости", ИнформацияОПрименимости);
		
		ПустаяПрименимость = Ложь;
		Если ЭтоБиблиотека
			И ИнформацияОПрименимости.ПрименимостьПатча.Количество() <= 1
			И ИнформацияОПрименимости.НеПрименим.Количество() <> 0 Тогда
			
			ПустаяПрименимость = Истина;
		КонецЕсли;
		...
		ДополнительныеПараметры.ПустаяПрименимость = ПустаяПрименимость;
	...
	Если Не ЭтоКонфигурацияБазовая Тогда 
		...
		Попытка
			Блокировка.Заблокировать();
			ПатчиСлужебный.ПодписатьПатчиДляБазовыхВерсий(ВерсияПатча, ПараметрыДляБазовых);
...
Функция ПересчетПрименимостиПатчаВручную(УникальныйИдентификатор) Экспорт
	...
	Для Каждого БазовыйПатч Из Запрос.Выполнить().Выгрузить() Цикл
		ИдентификаторПрограммы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БазовыйПатч.ПодписанДляКонфигурации, "ИдентификаторПрограммы");
		Результат = ПатчиСлужебный.ОбновлениеПрименимостиПатчаВСервисе(БазовыйПатч, Истина, ИдентификаторПрограммы);
		Результаты.Добавить(Результат);
	КонецЦикла;
	УстановитьПривилегированныйРежим(Ложь);

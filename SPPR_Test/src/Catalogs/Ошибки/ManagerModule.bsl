...
Функция СоздатьТаблицуОписанийОшибок() Экспорт
	...
	// Макаров - Добавлена колонка. По умолчанию не заполнялся
	Если ТаблицаОписанийОшибок.Колонки.Найти("ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников") = Неопределено Тогда
		ТаблицаОписанийОшибок.Колонки.Добавить("ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников", Новый ОписаниеТипов("Булево"));
	КонецЕсли; 
...
Процедура ЗаписатьОшибку(ДанныеОшибки, ПараметрыОшибки)
	...
	Если ЗначениеЗаполнено(ИзмененияПомещаютсяТолькоЧерезДругиеВетки) Тогда

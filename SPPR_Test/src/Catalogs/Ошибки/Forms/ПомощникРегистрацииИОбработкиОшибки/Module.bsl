...
Процедура ПротоколВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СтрокаТЧ <> Неопределено Тогда
		...
		ПараметрыФормы.Вставить("ВерсияИсправления", СтрокаТЧ.ВерсияИсправления);
		ПараметрыФормы.Вставить("СрочностьИсправления", СтрокаТЧ.СрочностьИсправления);
...
Процедура ДобавитьЗаписьВПротокол(АвторИзменений, Комментарий)
	...
	НоваяСтрока.ВерсияИсправления = Объект.ВерсияИсправления;
	НоваяСтрока.СрочностьИсправления = Объект.СрочностьИсправления;
...
Процедура ПередЗаписьюНаКлиенте()
	...
	Если Объект.Протокол.Количество()>0 Тогда
		...
		ВерсияИсправленияПоСтрокеПротокола = ПоследняяСтрока.ВерсияИсправления;
		СрочностьИсправленияПоСтрокеПротокола = ПоследняяСтрока.СрочностьИсправления;
	...
	Если Объект.Статус <> СтатусПоСтрокеПротокола ИЛИ Объект.КомуНаправлена <> КомуНаправленаПоСтрокеПротокола
		ИЛИ Объект.ВерсияИсправления <> ВерсияИсправленияПоСтрокеПротокола
		ИЛИ Объект.СрочностьИсправления <> СрочностьИсправленияПоСтрокеПротокола
...
Процедура ОткрытьСценарийПользователя(ТекстСсылки = Неопределено)
	...
	Если ТекстСсылки <> Неопределено Тогда
		...
		Если ХранитьСценарииРаботыПользователейВРепозитории Тогда
			...
			ПараметрВетка = ЗаменитьВеткуПриемникЕслиВеткаУдалена(ПараметрВетка);
...
&НаСервереБезКонтекста
Функция ЗаменитьВеткуПриемникЕслиВеткаУдалена(ТекВетка)
	ДанныеВетки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекВетка,"Приемник,Владелец,Имя");
	
	Попытка
		ДанныеРепозитория = ТестированиеЗапускТестирования.ИнформацияОВетке(ДанныеВетки.Владелец, ДанныеВетки.Имя);
		Возврат ТекВетка;
	Исключение
		
		ОшибкаКратко = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Если ТестированиеКлиентСервер.ТекстОшибкиВеткаНеНайдена(ОшибкаКратко) Тогда
			Возврат ДанныеВетки.Приемник;
		КонецЕсли;

...
// Получить ссылку на версию проекта по номеру.
// 	
// Параметры:
//  Проект - Строка - Имя проекта в котором искать версию
//  НомерВерсии - Строка - Номер версии
// 
// Возвращаемое значение:
//  СправочникСсылка.ВерсииПроекта, Неопределено 
Функция ПолучитьВерсиюПроектаПоНомеру(Проект, НомерВерсии) Экспорт
	...
	Запрос.Текст =
		...
		|	Справочник.ВерсииПроекта КАК ВерсииПроекта
		|ГДЕ
		|	ВерсииПроекта.Код = &Код
		|	И ВерсииПроекта.Владелец.Наименование = &Владелец";
	
	Запрос.УстановитьПараметр("Код", НомерВерсии);

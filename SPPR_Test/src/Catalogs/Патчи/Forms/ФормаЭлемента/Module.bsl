...
Процедура Подключаемый_ВерсииПатчаПриАктивизацииСтроки()
	...
	ПредупреждатьОПустойПрименимости = ТекущиеДанные <> Неопределено
		И Не ТекущиеДанные.ЭтоСтрокаПереключателя
		И Не ТекущиеДанные.ПатчДляБазовой
		И ТекущиеДанные.ПустаяПрименимость
		И (ТекущиеДанные.Состояние = ПредопределенноеЗначение("Перечисление.СтатусыПатчей.СозданНеПроверен")
			Или ТекущиеДанные.Состояние = ПредопределенноеЗначение("Перечисление.СтатусыПатчей.СозданПроверен"));
	...
	Элементы.ГруппаИнформацияПоПрименимости.Видимость      = ПредупреждатьОПустойПрименимости;
	Элементы.ГруппаИнформацияПоПрименимости1.Видимость     = ПредупреждатьОПустойПрименимости;
...
Процедура ОпубликоватьПатч(Перевыпуск = Ложь)
	...
	Иначе
		...
		ОткрытьФорму("Справочник.Патчи.Форма.ПодтверждениеПроверкиПатча", ПараметрыПроверки , ЭтотОбъект, , , , ОписаниеОповещения);
	КонецЕсли;
...
Процедура ОтметитьПроверкуПатча(Команда)
	...
	КонецЕсли;
	ПараметрыПроверки.Вставить("АвторИсправления", АвторИсправления);
	ПараметрыПроверки.Вставить("ПустаяПрименимость", Элементы.ВерсииПатчаДерево.ТекущиеДанные.ПустаяПрименимость);
...
Процедура ЗаполнитьВерсииПатча()
	...
	Для Каждого Запись Из ТаблицаЗаписей Цикл
		...
		Иначе
			...
			Если ПараметрыПатча.ПустаяПрименимость = Истина Тогда
				Строка.ПустаяПрименимость = Истина;
			Иначе
				Строка.ПустаяПрименимость = Ложь;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьПрименимостьПатча(Результат, ДополнительныеПараметры) Экспорт
	Применимость(Неопределено);
КонецПроцедуры

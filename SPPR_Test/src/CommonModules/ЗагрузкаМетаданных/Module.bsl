...
Процедура ЗагрузитьИзмененияВВеткахОсновнаяЗагрузка(ПараметрыGit, ВеткиПриемники, Проект, ДопПараметры)
	...
	Для Каждого КлючЗначение Из ВеткиПриемники Цикл
		...
		Для Каждого ДанныеВетки Из ДанныеВеткиПриемника Цикл
			...
			ОМСогласованиеНовыйОбъект = Новый ТаблицаЗначений;
			ОМСогласованиеНовыйОбъект.Колонки.Добавить("ОбъектМетаданных");
			ОМСогласованиеНовыйОбъект.Колонки.Добавить("ПодчиненныйОбъект");
			ОМСогласованиеНовыйОбъект.Индексы.Добавить("ОбъектМетаданных,ПодчиненныйОбъект");
			...
			Для Каждого ТекМетаданное Из ИзмененныеМетаданные Цикл
				...
				ПараметрыОтбора.Вставить("ОбъектМетаданных", ЗаписьРС.ОбъектМетаданных);
				ПараметрыОтбора.Вставить("ПодчиненныйОбъект", ЗаписьРС.ПодчиненныйОбъект);
				СтрокиОМСогласованиеНовыйОбъект = ОМСогласованиеНовыйОбъект.НайтиСтроки(ПараметрыОтбора);
				...
				Если ЗаписьРС.ТипИзменения = Перечисления.ТипыИзмененийОбъектовМетаданных.Добавлен
					...
					И (НЕ УжеЕстьПодробноеОписаниеИзменений ИЛИ СтрокиОМСогласованиеНовыйОбъект.Количество() > 0)
					Тогда		
					...
					Если СтрокиОМСогласованиеНовыйОбъект.Количество() = 0 Тогда
						//Защита, если УжеЕстьПодробноеОписаниеИзменений не пустое, т.к. по одному ОМ могли делать запись в РС несколько раз
						СтрокаОМСогласованиеНовыйОбъект = ОМСогласованиеНовыйОбъект.Добавить();
						СтрокаОМСогласованиеНовыйОбъект.ОбъектМетаданных = ЗаписьРС.ОбъектМетаданных;
						СтрокаОМСогласованиеНовыйОбъект.ПодчиненныйОбъект = ЗаписьРС.ПодчиненныйОбъект;
					КонецЕсли;
				...
				Если НадоСделатьУведомлениеНовоеМетаданное Тогда
					...
					Если ЗначениеЗаполнено(ОтветственныйЗаПроект) Тогда
						...
						Если ОтветственныеДляУведомленияНовыйОбъект[ЗаписьРС.Согласующий] = Неопределено Тогда         
							ТекстСообщения = СтрШаблон(НСтр("ru='Ответственный за проект <%1> получит уведомление о новом ОМ <%2> <%3>.'"), ЗаписьРС.Согласующий, ЗаписьРС.ОбъектМетаданных, ЗаписьРС.ПодчиненныйОбъект);
							ЗаписьЖурналаРегистрации("ЗагрузитьИзмененияВВетках",
														УровеньЖурналаРегистрации.Информация,
														,
														ДанныеВетки.Ссылка,
					...
					Иначе	
						...
						Если ДанныеТехПроектаПоВетке <> Неопределено И ЗначениеЗаполнено(ДанныеТехПроектаПоВетке.Ответственный) Тогда
							...
							Если ОтветственныеДляУведомленияНовыйОбъект[ЗаписьРС.Согласующий] = Неопределено Тогда         
								ТекстСообщения = СтрШаблон(НСтр("ru='Ответственный за технический проект <%1> получит уведомление о новом ОМ <%2> <%3>.'"), ЗаписьРС.Согласующий, ЗаписьРС.ОбъектМетаданных, ЗаписьРС.ПодчиненныйОбъект);
								ЗаписьЖурналаРегистрации("ЗагрузитьИзмененияВВетках",
															УровеньЖурналаРегистрации.Информация,
															,
															ДанныеВетки.Ссылка,
				...
				ИначеЕсли НадоСделатьУведомление Тогда
					Если ОтветственныеДляУведомленияИзменениеОбъекта[ЗаписьРС.Согласующий] = Неопределено Тогда         
						ТекстСообщения = СтрШаблон(НСтр("ru='Пользователь <%1> получит уведомление о изменении ОМ <%2> <%3>.'"), ЗаписьРС.Согласующий, ЗаписьРС.ОбъектМетаданных, ЗаписьРС.ПодчиненныйОбъект);
						ЗаписьЖурналаРегистрации("ЗагрузитьИзмененияВВетках",
													УровеньЖурналаРегистрации.Информация,
													,
													ДанныеВетки.Ссылка,
			...
			Если Найти(МассивСтрок[Сч], "new file mode 100644") > 0 И МассивСтрок.ВГраница() >= (Сч + 3) Тогда
				...
				ИначеЕсли ЭтоФайлМодуля(ТекСтр) Тогда
					Если ЗаписьРС.ПодчиненныйОбъект = Перечисления.ТипыМодулейОбъектовМетаданных.МодульМенеджера
						...
						ИЛИ ЭтоФайлОбщегоМодуля(ТекСтр)
						Тогда
							НайденоСозданиеОМ = Истина;
							Прервать;
					КонецЕсли;	
...
Функция ЭтоФайлОбщегоМодуля(ИмяФайла)
	
	Если Найти(НРег(ИмяФайла), "/commonmodules/") > 0 Тогда
		Возврат Истина;
	КонецЕсли;

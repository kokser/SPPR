...
Функция ПараметрыПатча(Параметры) Экспорт
	...
	НовыеПараметры.Вставить("ПустаяПрименимость", Неопределено);
...
Процедура РассчитатьПрименимостьИОпубликовать() Экспорт
	...
	Попытка
		...
		Для Каждого СтрокаПатч Из Патчи Цикл
			...
			ПрименитьРучноеИзменениеПрименимости(ПрименимостьДляПубликации, СтрокаПатч, ПрименимостьПолная);
...
Процедура ЗакончитьСозданиеПатча(ИнформацияОПатче)
	...
	Если Патч.Состояние <> Перечисления.СтатусыПатчей.Ошибка
		...
		СвойстваВерсииПатча = ЗначенияСвойствВерсииПатча(Патч.УникальныйИдентификатор, "РазделПроекта");
		ПараметрыРасчетаПрименимости = ПараметрыРасчетаПрименимости();
		ПараметрыРасчетаПрименимости.ЭтоБиблиотека = ИнформацияОПатче.ЭтоБиблиотека;
		ПараметрыРасчетаПрименимости.РазделПроекта = СвойстваВерсииПатча.РазделПроекта;
		ЗаполнитьЗначенияСвойств(ПараметрыРасчетаПрименимости, Патч);
		ИнформацияОПрименимости = ПрименимостьПатча(ПараметрыРасчетаПрименимости, Ложь);
		
		Если ИнформацияОПатче.ЭтоБиблиотека
			И ИнформацияОПрименимости.ПрименимостьПатча.Количество() <= 1
			И ИнформацияОПрименимости.НеПрименим.Количество() <> 0 Тогда
			
			Параметры.ПустаяПрименимость = Истина;
			Патч.Параметры = Новый ХранилищеЗначения(Параметры);
		КонецЕсли;
		
		ДобавитьПатчиДляБазовыхВерсий(Патчи, Патч, ИнформацияОПатче, ИнформацияОПрименимости);
	КонецЕсли;
...
Процедура ПодписатьПатчиДляБазовыхВерсий(ОсновнойПатч, ДополнительныеПараметры) Экспорт
	...
	Патч   = ДополнительныеПараметры.Патч;
	ДанныеПатча = ДополнительныеПараметры.ДанныеПатча;
	...
	ДополнительныеПараметры.Вставить("ПутьКПатчу", ПутьКПатчу);
	
	УстановитьПривилегированныйРежим(Истина);
	
	ДобавитьПатчиДляБазовыхВерсий(Патчи, ОсновнойПатч, ДополнительныеПараметры, ДополнительныеПараметры.ИнформацияОПрименимости);
	УстановитьПривилегированныйРежим(Ложь);

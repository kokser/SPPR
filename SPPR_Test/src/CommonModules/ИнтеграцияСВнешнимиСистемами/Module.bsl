...
Функция ЗарегистрироватьОшибку(ПараметрыОшибки, ОбъектыМетаданных) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыОшибки.МетодВыявленияОшибки) Тогда
		// Макаров - Добавил заполнение сразу из перечисления
		Если ТипЗнч(ПараметрыОшибки.МетодВыявленияОшибки) = Тип("ПеречислениеСсылка.МетодыВыявленияОшибок") Тогда		
			МетодВыявленияОшибки = ПараметрыОшибки.МетодВыявленияОшибки;	
		Иначе
	...
	Если ЗначениеЗаполнено(ПараметрыОшибки.СрочностьИсправления) Тогда
		// Макаров - Добавил заполнение сразу из перечисления
		Если ТипЗнч(ПараметрыОшибки.СрочностьИсправления) = Тип("ПеречислениеСсылка.СрочностьИсправленияОшибок") Тогда
			СрочностьИзОшибки = ПараметрыОшибки.СрочностьИсправления;	
		Иначе
	...
	// Макаров - Добавлено заполнение реквизита из параметра
	Если ПараметрыОшибки.Свойство("ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников") 
		И ЗначениеЗаполнено(ПараметрыОшибки.ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников) 
	Тогда
		Ошибка.ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников = ПараметрыОшибки.ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников;
	Иначе
		Ошибка.ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников = Ложь;	
	КонецЕсли;
...
Функция ИнициироватьПараметрыОшибки() Экспорт
	...
	// Макаров - добавлен параметр, который не заполнялся
	ПараметрыОшибки.Вставить("ОшибкаЗарегистрированаПоИнформацииОтВнешнихИсточников");
...
Функция ПодобратьДанныеВерсииПроекта(Проект, ПолныйНомерВерсии)
	...
	Запрос.УстановитьПараметр("ПолныйНомерВерсии", Строка(ПолныйНомерВерсии));  
...
Функция ЗаполнитьПараметрыОшибки(СтруктураОшибки, ИмяПроекта, ПостфиксПорядкаВоспроизведения, Версия)
	...
	Если ЗначениеЗаполнено(СтруктураОшибки.ВерсияФормата) Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ДополненнаяВерсия, "1.2.0.0") >= 0 Тогда
			...
			Если СтруктураОшибки.Свойство("КодСценария") Тогда

...
Функция УдалитьВеткуВGitLab(Проект, ИмяВетки) Экспорт
	...
	КонецПопытки;
	ОтветСтрокой = Ответ.ПолучитьТелоКакСтроку();
	Если Ответ.КодСостояния = 404 И ТестированиеКлиентСервер.ТекстОшибкиВеткаНеНайдена(ОтветСтрокой) Тогда
		ВызватьИсключение СтрШаблон(
...
Функция ДанныеОткрытогоЗапросаНаСлияниеПоВеткеСлужебный(Проект, ИмяВетки, ИмяВеткиПриемник) Экспорт
	...
	Исключение
		...
		Если ТестированиеКлиентСервер.ТекстОшибкиВеткаНеНайдена(ТекстОшибки) Тогда

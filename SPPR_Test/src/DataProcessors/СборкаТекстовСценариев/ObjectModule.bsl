...
Процедура ТекстЛинейногоСценарияПоСхемеРекурсивно(ПараметрыПолученияТекстаСценария,Отступ)
	...
	Для Каждого СтрокаДерева Из СхемаДерево.Строки Цикл
		...
		ИначеЕсли (СтрокаДерева.ТипЭлемента = ТипЭлементаШаблонСценария()) Тогда
			...
			Если НЕ ПараметрыПолученияТекстаСценария.Свойство("ИменаПодсценариевПриПолученииТекстаСценария") Тогда
				ПараметрыПолученияТекстаСценария.Вставить("ИменаПодсценариевПриПолученииТекстаСценария", Новый Соответствие);
			КонецЕсли;
			ИменаПодсценариевПриПолученииТекстаСценария = ПараметрыПолученияТекстаСценария.ИменаПодсценариевПриПолученииТекстаСценария;
			Если СтрокаДерева.ДополнительныеСвойства.Свойство("ЧтениеИзGit") И СтрокаДерева.ДополнительныеСвойства.ЧтениеИзGit Тогда
				ТекИмяПодсценария = СтрокаДерева.ДополнительныеСвойства.Шаблон;
			ИначеЕсли СтрокаДерева.ДополнительныеСвойства.Свойство("ЧтениеИзТекста") И СтрокаДерева.ДополнительныеСвойства.ЧтениеИзТекста Тогда
				ТекИмяПодсценария = СтрокаДерева.ДополнительныеСвойства.Шаблон;
			Иначе	
				ТекИмяПодсценария = СтрокаДерева.ДополнительныеСвойства.ИмяПодсценария;
			КонецЕсли;	
			
			СтрокаОпределяющаяЧтоПодсценарийУжеВызывался = СокрЛП(НРег(ТекИмяПодсценария));
			Если ИменаПодсценариевПриПолученииТекстаСценария[СтрокаОпределяющаяЧтоПодсценарийУжеВызывался] <> Неопределено Тогда
				ВызватьИсключение СтрШаблон(
				       НСтр("ru = 'Зацикливание при вызове сценария: %1'"), ТекИмяПодсценария);
			КонецЕсли;	
			ИменаПодсценариевПриПолученииТекстаСценария.Вставить(СтрокаОпределяющаяЧтоПодсценарийУжеВызывался, Истина);
			...
			ИменаПодсценариевПриПолученииТекстаСценария.Удалить(СтрокаОпределяющаяЧтоПодсценарийУжеВызывался);
			ПараметрыПолученияТекстаСценария.УровеньВложенности = ПараметрыПолученияТекстаСценария.УровеньВложенности - 1;
	...
	Для Каждого СтрТаблицаСтрок Из ТаблицаСтрок Цикл
		...
		ИначеЕсли СтрТаблицаСтрок.Оператор = ОператорПередачаПараметров()
			...
			Если ПараметрыПодсценария.Найти(ИмяПараметраНРег) = Неопределено Тогда
				...
				Иначе	
					ВызватьИсключение СтрШаблон(НСтр("ru = '<%3>. В подсценарии <%1> нет параметра с именем <%2>.'"),
					      ТекущаяСтрокаВложенногоСценария.ОписаниеЭлемента,ИмяПараметра, ТекущаяДата());
				КонецЕсли;	 
			КонецЕсли;	 
...
Процедура ОбработатьВыделениеЦветомУсловияЕслиИЦиклыПопытки(ТаблицаСтрок)
	...
	Для Каждого СтрТаблицаСтрок Из ТаблицаСтрок Цикл
		...
		ИначеЕсли СтрТаблицаСтрок.ТипЭлемента = ТипЭлементаБлочныйКомментарий() Тогда
			Продолжить;
		ИначеЕсли СтрТаблицаСтрок.ТипЭлемента = ТипЭлементаМногострочнаяСтрока() Тогда
			Продолжить;
		КонецЕсли;	 
...
Процедура ОтформатироватьПервыеСловаGherkin(ТаблицаСтрок)
	...
	КешТипЭлементаМногострочнаяСтрока = ТипЭлементаМногострочнаяСтрока();
	...
	Для Каждого СтрТаблицаСтрок Из ТаблицаСтрок Цикл
		...
		ИначеЕсли СтрТаблицаСтрок.ТипЭлемента = КешТипЭлементаБлочныйКомментарий Тогда
			Продолжить;
		ИначеЕсли СтрТаблицаСтрок.ТипЭлемента = КешТипЭлементаМногострочнаяСтрока Тогда
			Продолжить;
		КонецЕсли;	 
